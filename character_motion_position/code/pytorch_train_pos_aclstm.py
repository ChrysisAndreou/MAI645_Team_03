...
def train_one_iteraton(real_seq_np, model, optimizer, iteration, save_dance_folder, print_loss=False, save_bvh_motion=True):
    ...
    real_seq = torch.autograd.Variable(torch.FloatTensor(real_seq_dif_hip_x_z_np.tolist()).to(device))
 
    seq_len=real_seq.size()[1]-1
    in_real_seq=real_seq[:, 0:seq_len]

    predict_groundtruth_seq = torch.autograd.Variable(torch.FloatTensor(real_seq_dif_hip_x_z_np[:, 1:seq_len+1].tolist())).to(device).view(real_seq_np.shape[0], -1)
    ...
    if(save_bvh_motion==True):
        ...
        gt_seq = np.array(predict_groundtruth_seq[0].data.cpu().tolist()).reshape(-1,In_frame_size)
        last_x = 0.0
        last_z = 0.0
        ...
        out_seq=np.array(predict_seq[0].data.cpu().tolist()).reshape(-1,In_frame_size)
        last_x=0.0
        last_z=0.0
