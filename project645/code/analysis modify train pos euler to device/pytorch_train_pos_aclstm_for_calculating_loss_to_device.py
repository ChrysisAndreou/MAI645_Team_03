...
class acLSTM(nn.Module):
    def __init__(self, in_frame_size=171, hidden_size=1024, out_frame_size=171, device="cpu"):
        ...
        ##lstm#########################################################
        self.lstm1 = nn.LSTMCell(self.in_frame_size, self.hidden_size).to(self.device)
        self.lstm2 = nn.LSTMCell(self.hidden_size, self.hidden_size).to(self.device)
        self.lstm3 = nn.LSTMCell(self.hidden_size, self.hidden_size).to(self.device)
        self.decoder = nn.Linear(self.hidden_size, self.out_frame_size).to(self.device)
    
    
    #output: [batch*1024, batch*1024, batch*1024], [batch*1024, batch*1024, batch*1024]
    def init_hidden(self, batch):
        #c batch*(3*1024)
        c0 = torch.FloatTensor(np.zeros((batch, self.hidden_size))).to(self.device)
        c1= torch.FloatTensor(np.zeros((batch, self.hidden_size))).to(self.device)
        c2 = torch.FloatTensor(np.zeros((batch, self.hidden_size))).to(self.device)
        h0 = torch.FloatTensor(np.zeros((batch, self.hidden_size))).to(self.device)
        h1= torch.FloatTensor(np.zeros((batch, self.hidden_size))).to(self.device)
        h2= torch.FloatTensor(np.zeros((batch, self.hidden_size))).to(self.device)
    ...
    def forward(self, real_seq, condition_num=5, groundtruth_num=5):
        ...
        out_seq = torch.FloatTensor(np.zeros((batch,1))).to(self.device)

        out_frame = torch.FloatTensor(np.zeros((batch,self.out_frame_size))).to(self.device)
...
def train_one_iteraton(real_seq_np, model, optimizer, iteration, save_dance_folder, device, print_loss=False, save_bvh_motion=True):
    ...
    real_seq = torch.FloatTensor(real_seq_dif_hip_x_z_np.tolist()).to(device)
 
    seq_len=real_seq.size()[1]-1
    in_real_seq=real_seq[:, 0:seq_len]

    predict_groundtruth_seq = torch.FloatTensor(real_seq_dif_hip_x_z_np[:, 1:seq_len+1].tolist()).to(device).view(real_seq_np.shape[0], -1)
...
def train(dances, frame_rate, batch, seq_len, read_weight_path, write_weight_folder,
          ...
          num_iterations_for_session=100000, resume_from_iteration_num=0, device="cpu"):
    
    seq_len=seq_len+2
    # torch.cuda.set_device(0) # Removed: device will be handled by the 'device' parameter

    model = acLSTM(in_frame_size=in_frame, hidden_size=hidden_size, out_frame_size=out_frame, device=device)
    
    if(read_weight_path!=""):
        # model.load_state_dict(torch.load(read_weight_path)) # Load to CPU first, then move
        model.load_state_dict(torch.load(read_weight_path, map_location='cpu'))
    ...
    for i in range(num_iterations_for_session):   
        ...
        train_one_iteraton(dance_batch_np, model, optimizer, iteration, write_bvh_motion_folder, device, print_loss, save_bvh_motion)
...
def main():
    ...
    parser.add_argument('--device', type=str, default=None, help='Device to use for training (e.g., "cpu", "cuda", "cuda:0")')
    ...
    train(dances, args.dance_frame_rate, args.batch_size, args.seq_len, args.read_weight_path, args.write_weight_folder,
          ...
          num_iterations_for_session=args.total_iterations, resume_from_iteration_num=resume_from_iteration_val, device=device)
