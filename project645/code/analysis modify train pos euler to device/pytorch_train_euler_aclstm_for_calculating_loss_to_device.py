...
class acLSTM(nn.Module):
    ...
    def forward(self, real_seq, condition_num, groundtruth_num):
        ...
        device = real_seq.device # Use device of the input sequence
        
        condition_lst = self.get_condition_lst(condition_num, groundtruth_num, seq_len)
        (vec_h, vec_c) = self.init_hidden(batch_size) # These will be on model's device
        
        # Ensure vec_h and vec_c are on the same device as real_seq if they were not already
        # This might be redundant if model.to(device) was called correctly before forward
        vec_h = [h.to(device) for h in vec_h]
        vec_c = [c.to(device) for c in vec_c]

        out_seq_list = []
        # Initialize out_frame for the first conditioned step if seq starts with it
        out_frame = torch.zeros(batch_size, self.out_frame_size, device=device)
