...
def generate_seq(initial_seq_np, generate_frames_number, model, save_dance_folder, 
    ...
    for b in range(batch):
        ...
        # Quantitative evaluation for each batch item if data is available
        if dances is not None and dance_info is not None and quantitative_comparison_len > 0:
            if b < len(dance_info): # Ensure dance_info has an entry for current batch item b
                ...
                if dance_id < len(dances):
                    ...
                    print(f"Batch {b} - Quantitative Evaluation: dance_id {dance_id} out of bounds for dances list (len {len(dances)}).")
            else:
                print(f"Batch {b} - Quantitative Evaluation: dance_info missing for this batch index.")

        # Save ground truth BVH for each batch item for comparison
        if dances is not None and dance_info is not None:
            if b < len(dance_info): # Ensure dance_info has an entry for current batch item b
                ...
                if dance_id < len(dances):
                    ...
                    print(f"Batch {b} - Could not save ground truth: dance_id {dance_id} out of bounds for dances list (len {len(dances)}).")
            else:
                print(f"Batch {b} - Could not save ground truth: dance_info missing for this batch index.")
...
if __name__ == '__main__':
    ...
    dance_len_lst = get_dance_len_lst(dances_list)
